...
class ImportOldSurNameData implements WithHeadingRow,ToCollection
{
    ...
    */
    ...
    {
        Surname::where('old_client_id',$this->old_client_id)->delete();
        $saveDate = [];
        foreach ($rows as $row) {
        ...
        if (!empty($saveDate) && count($saveDate) > 0) {
            foreach (array_chunk($saveDate,1500) as $chunk) {
                DB::connection('lifecell_lic')->table('surname')->insert($chunk);
            }
        }
    }
}
